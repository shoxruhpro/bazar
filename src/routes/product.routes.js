...
const createSchema = Joi.object({
    ...
    variants: Joi.array().items(Joi.string().max(100)).max(10).allow(null).allow(),
...
const updateSchema = Joi.object({
    ...
    email: Joi.string().min(9).max(255).allow(null).allow(''),
    variants: Joi.array().items(Joi.string().max(100)).max(10).allow(null).allow(),
    photos: Joi.array().items(Joi.string().min(17).max(255)).min(1).max(5).allow(null),
    ...
    .get(async (req, res) => {
        try {
            ...
            const products = await db.manyOrNone(
                ...
                "t.uz AS tariff_uz, t.ru AS tariff_ru, t.en AS tariff_en, t.tariff_id " +
                ...
                filter + ' ORDER BY t.price', req.query)
